...
Les paramètres Docaposte se trouvent dans le fichier `application.properties` :
...
ESUP-Stage met à disposition des api "public" accessible avec un des tokens paramétrés dans `appli.tokens` (anciennement `appli.public.tokens` avant la version 3). La liste des api se trouve dans `/public/swagger-ui.html`.
...
[,properties]
----
# token à renseigner du côté de l'application de signature électronique permettant d'autoriser les webhook à accéder aux api /public/api d'esup-stage séparés par des ; (exemple : token1;token2;token3). Ce token doit être créé par l'administrateur.
appli.tokens=xxxx
...
webhook.signature.token=yyyyy # Token fournit par l'application de signature électronique.
### -------------------- ###

### Paramétrage esup-signature ###
# numero du circuit
esupsignature.uri=http://localhost:8880/ws
esupsignature.circuit=123 # Peut ne pas être utilisé. Dans ce cas renseigner le circuit au niveau du centre de gestion.
### -------------------- ###
----
