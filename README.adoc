...
* Java OpenJdk 21
* Apache Maven
* Apache Tomcat 11 (requis depuis Spring Boot 4 : Servlet 6.1)
...
WARNING: Si vous aviez déjà une version antérieure d'ESUP STAGE et que vous en installer une nouvelle, il est fortement recommandé de supprimer le package-lock.json et les node_modules dans le dossier frontend avant de lancer la compilation.
...
Après la compilation, le fichier généré est le suivant : `/opt/estage/target/esup-stage-<version>.war`

IMPORTANT: avec Tomcat, le chemin d'accès de l'application dépend du nom du fichier copié dans le répertoire `webapps` :

* `ROOT.war` -> application accessible à la racine `/`
* `esup-stage.war` -> application accessible sous `/esup-stage`

IMPORTANT: si vous copiez directement le fichier `esup-stage-<version>.war` sans le renommer, Tomcat exposera l'application sous un contexte incluant la version, par exemple `/esup-stage-3.3.0`.
...
ESUP STAGE a été déployé et testé sur une version TOMCAT 11 (requis depuis Spring Boot 4 : Servlet 6.1)

TIP: Vous pouvez télécharger tomcat ici : https://tomcat.apache.org/download-11.cgi
...
//tar xzf apache-tomcat-11.0.25.tar.gz

//ln -sfn apache-tomcat-11.0.25 tomcat-esup-stage
...
Supprimer (ou déplacer une sauvegarde) votre répertoire `/opt/tomcat-esup-stage/webapp/esup-stage` avant le déploiement si vous souhaitez un accès via `/esup-stage`
ou `/opt/tomcat-esup-stage/webapp/ROOT` si vous souhaitez un accès à la racine `/`

Copier directement votre fichier `/opt/estage/target/esup-stage-<version>.war` dans votre répertoire *`webapp` de tomcat* en le renommant selon le chemin souhaité :

* `esup-stage.war` pour un accès via `/esup-stage`
* `ROOT.war` pour un accès à la racine `/`

[source,shell]
----
cp /opt/esup-stage/target/esup-stage-3.0.0.4.war /opt/tomcat-esup-stage/webapps/esup-stage.war
----
...
Bravo, l'installation est terminée ! Vous pouvez y accéder sur `http://localhost:8080/esup-stage` si vous avez déployé `esup-stage.war`, ou sur `http://localhost:8080/` si vous avez déployé `ROOT.war`

Lancement direct du `war` (ne pas utiliser : en cours de debug) `java -jar /opt/estage/target/esup-stage-<version>.war`
